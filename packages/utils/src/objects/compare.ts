...
export const compareObjects = (a: any, b: any, visited = new WeakMap()): boolean => {
  ...
  if (Array.isArray(a) && Array.isArray(b)) {
    ...
    for (let i = 0; i < a.length; i++) {
      if (!compareObjects(a[i], b[i], visited)) return false;
    }
    return true;
  }
  ...
  if (a instanceof Map && b instanceof Map) {
    ...
    for (const [key, value] of a) {
      if (!b.has(key) || !compareObjects(value, b.get(key), visited)) return false;
    }
    return true;
  }
  ...
  for (const key of keysA) {
    if (!keysB.includes(key) || !compareObjects(a[key], b[key], visited)) {
      return false;
    }
  }
  return true;
